...
class Network:
    """A simulation of a neural network using objects to represent neurons and the synapses between them. Use the following parameters:
        ...
        [9] path (str) - Where the network should look for images and folders and such"""
    def __init__(self, dimensions, random_weights=True, threshold=None, threshold_change=None, bias_node=False, path=""):
    ...
    #Incomplete, to be worked on later
    ...
    def export_graph(self, print_values=True):
        ...
        if self.img_name:
            os.remove(self.path + "/static/diagrams/"+self.img_name)
        self.img_name="plot" + str(random.randint(1, 100000)) + ".png"
        plt.savefig(self.path + "/static/diagrams/" + self.img_name, bbox_inches="tight", pad_inches=0)

        im = Image.open(self.path + "/static/diagrams/" + self.img_name)
        im = self.trim(im)
        im.save(self.path + "/static/diagrams/" + self.img_name)
